export const resources = {
  en: {
    translation: {
      // Login & Registration
      missingInformation: "Missing Information",
      fillAllFields: "Please fill in all required fields.",
      weakPassword: "Weak Password",
      weakPasswordDesc: "Password must be at least 8 characters long.",
      passwordMismatch: "Password Mismatch",
      passwordsDoNotMatch:
        "Passwords do not match. Please verify and try again.",
      termsRequired: "Terms Required",
      agreeTermsToProceed: "Please agree to the Terms of Service to proceed.",
      createYourAccount: "Create your\naccount.",
      startTrackingExpenses: "Start tracking your expenses today.",
      fullName: "FULL NAME",
      emailLabel: "EMAIL",
      monthlyIncomeLabel: "MONTHLY INCOME (SALARY / WAGE)",
      passwordLabel: "PASSWORD",
      confirmPasswordLabel: "CONFIRMER PASSWORD",
      hide: "hide",
      show: "show",
      agreeTo: "I agree to the ",
      termsOfService: "Terms of Service",
      and: " and ",
      privacyPolicy: "Privacy Policy",
      createAccount: "Create Account",
      alreadyHaveAccount: "Already have an account? ",
      signIn: "Sign In",
      welcome: "Ledger\n",
      signInToAccount: "Personal Financial Management & Expense Tracking",
      forgotPassword: "Forgot password?",
      orContinueWith: "or continue with",
      dontHaveAccount: "Don't have an account? ",
      signUp: "Sign Up",
      fullNamePlaceholder: "Enter your full name",
      emailPlaceholder: "Enter your e-mail address",
      incomePlaceholder: "e.g. 3000.00",
      passwordPlaceholder: "Min. 8 characters",
      loginPasswordPlaceholder: "Enter your password",
      confirmPasswordPlaceholder: "Repeat your password",
      forgotPasswordSubtitle:
        "Enter your email address to receive a password reset code.",
      resetCodeSent:
        "If an account exists, a password reset link/code has been sent.",
      genericError: "Something went wrong. Please try again.",
      sendResetCode: "Send Reset Code",
      ok: "OK",

      // Reset Password Flow
      resetPassword: "Reset Password",
      setNewPassword: "Set New Password",
      resetPasswordSubtitle: "Enter the 6-digit code sent to your email.",
      setNewPasswordSubtitle: "Code verified! Enter your new password below.",
      verificationCode: "VERIFICATION CODE",
      enterVerificationCode: "Please enter the verification code.",
      enterNewPassword: "Please enter your new password.",
      passwordResetSuccess: "Password updated successfully!",
      verifyCode: "Verify Code",
      rememberPassword: "Remember your password?",

      // Settings & Preferences
      settings: "Settings",
      account: "ACCOUNT",
      preferences: "PREFERENCES",
      language: "Language",
      monthlyBudget: "Monthly Budget",
      darkMode: "Dark Mode",
      signOut: "Sign Out",
      deleteAccount: "Delete Account",
      selectLanguage: "Select Language",
      cancel: "Cancel",
      edit: "Edit",
      personalInfo: "Personal Info",
      changePassword: "Change Password",
      paymentMethods: "Payment Methods",
      savedMethods: "SAVED METHODS",
      addPaymentMethod: "Add Payment Method",
      currency: "Currency",
      accountActions: "ACCOUNT ACTIONS",
      sessionExpired: "Session Expired",
      pleaseLogInAgain: "Please log in again.",
      signOutConfirm: "Are you sure you want to sign out?",
      deleteAccountConfirm:
        "Are you sure you want to permanently delete your account?",
      delete: "Delete",
      error: "Error",
      success: "Success",
      couldNotDeleteAccount: "Could not delete account.",
      user: "User",
      lightMode: "Light Mode",
      changeMonthlyIncome: "Change Monthly Income",
      monthlyIncomeDesc:
        "Enter your new monthly income for budget calculations.",
      monthlyIncomePlaceholder: "$ 0.00",
      currentName: "Full Name",
      currentMail: "E-mail",
      currentPassword: "Current Password",
      currentPasswordPlaceholder: "Enter current password",
      newPassword: "New Password",
      newPasswordPlaceholder: "Enter new password",
      confirmNewPassword: "Confirm New Password",
      updatePassword: "Update Password",
      save: "Save",
      passwordChanged: "Your password has been changed successfully.",
      couldNotUpdateIncome: "Failed to update monthly income.",
      couldNotChangePassword:
        "Failed to change password. Please check current password.",
      invalidIncome: "Please enter a valid income amount.",
      wageRaise: "Monthly Income Change",
      typoFix: "Fix Typo",

      // Overview Screen
      overview: "OVERVIEW",
      hello: "Hello",
      takeCareFinances: "Take good care of your finances!",
      totalBalance: "TOTAL BALANCE",
      investmentPot: "Investment Pot",
      income: "INCOME",
      expenses: "EXPENSES",
      spendingTrend: "Spending Trend",
      recent: "Recent",
      seeAll: "See all",
      deposit: "Deposit",
      cash: "Cash",
      card: "Card",
      noCardsAvailable: "(No Cards Available)",
      cardInUseWarning:
        "This card is associated with other expenses. Are you sure you want to delete it?",
      deleteCardConfirmation: "Are you sure you want to delete this card?",
      deletedCard: "Deleted Card",

      // Categories
      food: "Food",
      housing: "Housing",
      transportation: "Transportation",
      entertainment: "Entertainment",
      fixed_expenses: "Fixed Expenses",
      healthcare: "Healthcare",
      clothing: "Clothing",
      pet: "Pet",
      travel: "Travel",
      others: "Others",

      // Card Management Modal
      registeredCards: "Registered Cards",
      addCard: "Add Card",
      editCard: "Edit Card",
      updateCard: "Update Card",
      noCardsRegistered: "No cards registered",
      close: "Close",
      back: "Back",
      newCard: "New Card",
      cardName: "Card Name",
      cardNamePlaceholder: "e.g. Chase Sapphire, Nubank",
      enterCardName: "Please enter a card name.",
      failedToFetchCards: "Failed to fetch registered cards.",
      failedToSaveCard: "Failed to save card details.",
      failedToDeleteCard: "Failed to delete card.",
      cardType: "Card Type",
      saveCard: "Save Card",
      credit: "Credit",
      debit: "Debit",

      // Income Modal
      addIncomeDeposit: "Add Income Deposit",
      sourceDescription: "Source / Description",
      amount: "Amount",
      addToIncome: "Add to Income",

      // Expense Modal
      addNewExpense: "Add New Expense",
      description: "Description",
      descriptionPlaceholder: "e.g. Grocery Shopping",
      details: "Details",
      category: "Category",
      paymentMethod: "Payment Method",
      selectCard: "Select Card",
      repeat: "Repeat",
      markAsPaid: "Mark as Paid",
      saveExpense: "Save Expense",
      none: "None",
      weekly: "Weekly",
      monthly: "Monthly",
      yearly: "Yearly",
      date: "Date",

      // Transactions Screen
      transactions: "Transactions",
      records: "records",
      in: "IN",
      out: "OUT",
      net: "NET",
      all: "All",
      allPaymentMethods: "All Payment Methods",
      income_transaction: "Income",
      expense: "Expense",
      incomeDeposit: "Income Deposit",
      noTransactionsRegistered: "No transactions registered.",

      // Analytics
      analytics: "Analytics",
      spendingInsights: "Spending insights",
      incomeVsExpenses: "Income vs. Expenses",
      last6Months: "Last 6 months",
      paymentMethodBreakdown: "Payment Method Breakdown",
      cardVsCash: "Card vs. Cash",
      cardExpenses: "Card Expenses",
      cashExpenses: "Cash Expenses",
      ofTotal: "of total",
      spendingByCategory: "Spending by Category",
      total: "Total",
      noExpenses: "No Expenses",
      netSavingsTrend: "Net Savings Trend",
      netSavings: "NET SAVINGS",
      health: "Health",
      shopping: "Shopping",
      general: "General",
      cardsUsage: "Card Breakdown",
      savingsAnalysis: "Savings & Financial Summary",
      incomeVsExpenseDetails: "Income vs. Expense Insights",
      paymentAnalysis: "Payment Methods Deep Dive",
      categoryAnalysis: "Category Breakdown Insights",
      savedSoFar: "Total Saved So Far (6 Months)",
      avgPerMonth: "Average monthly savings",
      monthlySavingsHistory: "6-Month Savings History",
      savingsTip: "Savings Rate",
      ofIncomeSavedThisMonth: "of income saved this month.",
      currentMonthNet: "Selected Month Net Balance",
      comparisonSummary: "Period Summary",
      totalIncome: "Total Income (6 Months)",
      totalExpenses: "Total Expenses (6 Months)",
      highestSavingsMonth: "Best Savings Month",
      methodBreakdown: "Payment Overview",
      categoryDistribution: "Category Analysis",
      noCategoryData: "No category expense records found.",
      creditVsDebit: "Credit vs. Debit Breakdown",
      creditExpenses: "Credit Cards",
      debitExpenses: "Debit Cards",
    },
  },
  pt: {
    translation: {
      // Login e Cadastro
      missingInformation: "Informações Faltando",
      fillAllFields: "Por favor, preencha todos os campos obrigatórios.",
      weakPassword: "Senha Fraca",
      weakPasswordDesc: "A senha deve ter pelo menos 8 caracteres.",
      passwordMismatch: "Senhas Não Coincidem",
      passwordsDoNotMatch:
        "As senhas não coincidem. Verifique e tente novamente.",
      termsRequired: "Termos Obrigatórios",
      agreeTermsToProceed:
        "Por favor, aceite os Termos de Serviço para continuar.",
      createYourAccount: "Crie a sua\nconta.",
      startTrackingExpenses: "Comece a acompanhar suas despesas hoje.",
      fullName: "NOME COMPLETO",
      emailLabel: "E-MAIL",
      monthlyIncomeLabel: "RENDA MENSAL (SALÁRIO)",
      passwordLabel: "SENHA",
      confirmPasswordLabel: "CONFIRMAR SENHA",
      hide: "ocultar",
      show: "mostrar",
      agreeTo: "Eu concordo com os ",
      termsOfService: "Termos de Serviço",
      and: " e a ",
      privacyPolicy: "Política de Privacidade",
      createAccount: "Criar Conta",
      alreadyHaveAccount: "Já tem uma conta? ",
      signIn: "Entrar",
      welcome: "Ledger\n",
      signInToAccount: "Gestão Financeira Pessoal e Controle de Despesas",
      forgotPassword: "Esqueceu a senha?",
      orContinueWith: "ou continue com",
      dontHaveAccount: "Não tem uma conta? ",
      signUp: "Cadastrar-se",
      fullNamePlaceholder: "Digite seu nome completo",
      emailPlaceholder: "Digite seu endereço de e-mail",
      incomePlaceholder: "ex. 3000,00",
      passwordPlaceholder: "Mín. 8 caracteres",
      loginPasswordPlaceholder: "Digite sua senha",
      confirmPasswordPlaceholder: "Repita sua senha",
      forgotPasswordSubtitle:
        "Informe seu endereço de e-mail para receber um código de redefinição de senha.",
      resetCodeSent:
        "Se existir uma conta associada, um link/código de redefinição de senha foi enviado.",
      genericError: "Algo deu errado. Por favor, tente novamente.",
      sendResetCode: "Enviar Código de Redefinição",
      ok: "OK",

      // Redefinição de Senha
      resetPassword: "Redefinir Senha",
      setNewPassword: "Definir Nova Senha",
      resetPasswordSubtitle:
        "Digite o código de 6 dígitos enviado ao seu e-mail.",
      setNewPasswordSubtitle:
        "Código verificado! Digite sua nova senha abaixo.",
      verificationCode: "CÓDIGO DE VERIFICAÇÃO",
      enterVerificationCode: "Por favor, digite o código de verificação.",
      enterNewPassword: "Por favor, digite sua nova senha.",
      passwordResetSuccess: "Senha atualizada com sucesso!",
      verifyCode: "Verificar Código",
      rememberPassword: "Lembrou da sua senha?",

      // Configurações e Preferências
      settings: "Configurações",
      account: "CONTA",
      preferences: "PREFERÊNCIAS",
      language: "Idioma",
      monthlyBudget: "Orçamento Mensal",
      darkMode: "Modo Escuro",
      signOut: "Sair",
      deleteAccount: "Excluir Conta",
      selectLanguage: "Selecionar Idioma",
      cancel: "Cancelar",
      edit: "Editar",
      personalInfo: "Informações Pessoais",
      changePassword: "Alterar Senha",
      paymentMethods: "Formas de Pagamento",
      savedMethods: "MÉTODOS SALVOS",
      addPaymentMethod: "Adicionar Forma de Pagamento",
      currency: "Moeda",
      accountActions: "AÇÕES DA CONTA",
      sessionExpired: "Sessão Expirada",
      pleaseLogInAgain: "Por favor, faça login novamente.",
      signOutConfirm: "Tem certeza de que deseja sair?",
      deleteAccountConfirm:
        "Tem certeza de que deseja excluir permanentemente sua conta?",
      delete: "Excluir",
      error: "Erro",
      success: "Sucesso",
      couldNotDeleteAccount: "Não foi possível excluir a conta.",
      user: "Usuário",
      lightMode: "Modo Claro",
      changeMonthlyIncome: "Alterar Renda Mensal",
      monthlyIncomeDesc:
        "Informe sua nova renda mensal para os cálculos de orçamento.",
      monthlyIncomePlaceholder: "R$ 0,00",
      currentName: "Nome Completo",
      currentMail: "E-mail",
      currentPassword: "Senha Atual",
      currentPasswordPlaceholder: "Digite a senha atual",
      newPassword: "Nova Senha",
      newPasswordPlaceholder: "Digite a nova senha",
      confirmNewPassword: "Confirmar Nova Senha",
      updatePassword: "Atualizar Senha",
      save: "Salvar",
      passwordChanged: "Sua senha foi alterada com sucesso.",
      couldNotUpdateIncome: "Falha ao atualizar a renda mensal.",
      couldNotChangePassword:
        "Falha ao alterar a senha. Verifique a senha atual.",
      invalidIncome: "Por favor, insira um valor de renda válido.",
      wageRaise: "Alteração de Renda Mensal",
      typoFix: "Corrigir Erro de Digitação",

      // Tela Visão Geral
      overview: "VISÃO GERAL",
      hello: "Olá",
      takeCareFinances: "Cuide bem das suas finanças!",
      totalBalance: "SALDO TOTAL",
      investmentPot: "Reserva de Investimentos",
      income: "RECEITA",
      expenses: "DESPESAS",
      spendingTrend: "Tendência de Gastos",
      recent: "Recentes",
      seeAll: "Ver todos",
      deposit: "Depósito",
      cash: "Dinheiro / PIX",
      card: "Cartão",
      noCardsAvailable: "(Sem cartões disponíveis)",
      cardInUseWarning:
        "Este cartão está associado a outras despesas. Tem certeza de que deseja excluí-lo?",
      deleteCardConfirmation: "Tem certeza de que deseja excluir este cartão?",
      deletedCard: "Cartão Excluído",

      // Categorias
      food: "Alimentação",
      housing: "Moradia",
      transportation: "Transporte",
      entertainment: "Lazer",
      fixed_expenses: "Despesas Fixas",
      healthcare: "Saúde",
      clothing: "Vestuário",
      pet: "Pets",
      travel: "Viagens",
      others: "Outros",

      // Modal de Cartões
      registeredCards: "Cartões Cadastrados",
      addCard: "Adicionar Cartão",
      editCard: "Editar Cartão",
      updateCard: "Atualizar Cartão",
      noCardsRegistered: "Nenhum cartão cadastrado",
      close: "Fechar",
      back: "Voltar",
      newCard: "Novo Cartão",
      cardName: "Nome do Cartão",
      cardNamePlaceholder: "ex. Itaú, Nubank",
      enterCardName: "Por favor, insira um nome para o cartão.",
      failedToFetchCards: "Falha ao buscar cartões cadastrados.",
      failedToSaveCard: "Falha ao salvar detalhes do cartão.",
      failedToDeleteCard: "Falha ao excluir cartão.",
      cardType: "Tipo de Cartão",
      saveCard: "Salvar Cartão",
      credit: "Crédito",
      debit: "Débito",

      // Modal de Receitas
      addIncomeDeposit: "Adicionar Depósito de Receita",
      sourceDescription: "Origem / Descrição",
      amount: "Valor",
      addToIncome: "Adicionar à Receita",

      // Modal de Despesas
      addNewExpense: "Adicionar Nova Despesa",
      description: "Descrição",
      descriptionPlaceholder: "ex. Compras no Supermercado",
      details: "Detalhes",
      category: "Categoria",
      paymentMethod: "Forma de Pagamento",
      selectCard: "Selecionar Cartão",
      repeat: "Repetição",
      markAsPaid: "Marcar como Pago",
      saveExpense: "Salvar Despesa",
      none: "Nenhum",
      weekly: "Semanal",
      monthly: "Mensual",
      yearly: "Anual",
      date: "Data",

      // Tela de Transações
      transactions: "Transações",
      records: "registros",
      in: "ENTRADA",
      out: "SAÍDA",
      net: "LÍQUIDO",
      all: "Todos",
      allPaymentMethods: "Todas as Formas de Pagamento",
      income_transaction: "Depósito",
      expense: "Despesa",
      incomeDeposit: "Depósito de Receita",
      noTransactionsRegistered: "Nenhuma transação registrada.",

      // Análises
      analytics: "Visão Analítica",
      spendingInsights: "Análises de gastos",
      incomeVsExpenses: "Receita vs. Despesas",
      last6Months: "Últimos 6 meses",
      paymentMethodBreakdown: "Detalhamento por Forma de Pagamento",
      cardVsCash: "Cartão vs. Dinheiro / PIX",
      cardExpenses: "Despesas no Cartão",
      cashExpenses: "Despesas em Dinheiro / PIX",
      ofTotal: "do total",
      spendingByCategory: "Gastos por Categoria",
      total: "Total",
      noExpenses: "Sem Despesas",
      netSavingsTrend: "Tendência de Economia Líquida",
      netSavings: "ECONOMIA LÍQUIDA",
      health: "Saúde",
      shopping: "Compras",
      general: "Geral",
      cardsUsage: "Detalhamento por Cartão",
      savingsAnalysis: "Resumo Financeiro e Economias",
      incomeVsExpenseDetails: "Análise de Receita vs. Despesas",
      paymentAnalysis: "Análise Detalhada de Formas de Pagamento",
      categoryAnalysis: "Análise por Categoria",
      savedSoFar: "Total Economizado até Agora (6 Meses)",
      avgPerMonth: "Média de economia mensal",
      monthlySavingsHistory: "Histórico de Economia de 6 Meses",
      savingsTip: "Taxa de Economia",
      ofIncomeSavedThisMonth: "da renda economizada este mês.",
      currentMonthNet: "Saldo Líquido do Mês Selecionado",
      comparisonSummary: "Resumo do Período",
      totalIncome: "Receita Total (6 Meses)",
      totalExpenses: "Despesas Totais (6 Meses)",
      highestSavingsMonth: "Mês com Maior Economia",
      methodBreakdown: "Visão Geral de Pagamentos",
      categoryDistribution: "Análise de Categorias",
      noCategoryData: "Nenhum registro de despesa por categoria encontrado.",
      creditVsDebit: "Detalhamento Crédito vs. Débito",
      creditExpenses: "Cartões de Crédito",
      debitExpenses: "Cartões de Débito",
    },
  },
  es: {
    translation: {
      // Inicio de Sesión y Registro
      missingInformation: "Información Faltante",
      fillAllFields: "Por favor, complete todos los campos requeridos.",
      weakPassword: "Contraseña Débil",
      weakPasswordDesc: "La contraseña debe tener al menos 8 caracteres.",
      passwordMismatch: "Las Contraseñas No Coinciden",
      passwordsDoNotMatch:
        "Las contraseñas no coinciden. Verifique e intente nuevamente.",
      termsRequired: "Términos Requeridos",
      agreeTermsToProceed:
        "Por favor, acepte los Términos de Servicio para continuar.",
      createYourAccount: "Crea tu\ncuenta.",
      startTrackingExpenses: "Comienza a controlar tus gastos hoy.",
      fullName: "NOMBRE COMPLETO",
      emailLabel: "CORREO ELECTRÓNICO",
      monthlyIncomeLabel: "INGRESO MENSUAL (SALARIO)",
      passwordLabel: "CONTRASEÑA",
      confirmPasswordLabel: "CONFIRMAR CONTRASEÑA",
      hide: "ocultar",
      show: "mostrar",
      agreeTo: "Acepto los ",
      termsOfService: "Términos de Servicio",
      and: " y la ",
      privacyPolicy: "Política de Privacidad",
      createAccount: "Crear Cuenta",
      alreadyHaveAccount: "¿Ya tienes una cuenta? ",
      signIn: "Iniciar Sesión",
      welcome: "Ledger\n",
      signInToAccount: "Gestión Financiera Personal y Control de Gastos",
      forgotPassword: "¿Olvidaste tu contraseña?",
      orContinueWith: "o continúa con",
      dontHaveAccount: "¿No tienes una cuenta? ",
      signUp: "Registrarse",
      fullNamePlaceholder: "Ingresa tu nombre completo",
      emailPlaceholder: "Ingresa tu dirección de correo electrónico",
      incomePlaceholder: "ej. 3000,00",
      passwordPlaceholder: "Mín. 8 caracteres",
      loginPasswordPlaceholder: "Ingresa tu contraseña",
      confirmPasswordPlaceholder: "Repite tu contraseña",
      forgotPasswordSubtitle:
        "Ingresa tu correo electrónico para recibir un código de restablecimiento de contraseña.",
      resetCodeSent:
        "Si existe una cuenta, se ha enviado un enlace/código de restablecimiento de contraseña.",
      genericError: "Algo salió mal. Por favor, inténtalo de nuevo.",
      sendResetCode: "Enviar Código de Restablecimiento",
      ok: "Aceptar",

      // Restablecer Contraseña
      resetPassword: "Restablecer Contraseña",
      setNewPassword: "Establecer Nueva Contraseña",
      resetPasswordSubtitle:
        "Ingresa el código de 6 dígitos enviado a tu correo.",
      setNewPasswordSubtitle:
        "¡Código verificado! Ingresa tu nueva contraseña a continuación.",
      verificationCode: "CÓDIGO DE VERIFICACIÓN",
      enterVerificationCode: "Por favor, ingresa el código de verificación.",
      enterNewPassword: "Por favor, ingresa tu nueva contraseña.",
      passwordResetSuccess: "¡Contraseña actualizada con éxito!",
      verifyCode: "Verificar Código",
      rememberPassword: "¿Recuerdas tu contraseña?",

      // Ajustes y Preferencias
      settings: "Ajustes",
      account: "CUENTA",
      preferences: "PREFERENCIAS",
      language: "Idioma",
      monthlyBudget: "Presupuesto Mensual",
      darkMode: "Modo Oscuro",
      signOut: "Cerrar Sesión",
      deleteAccount: "Eliminar Cuenta",
      selectLanguage: "Seleccionar Idioma",
      cancel: "Cancelar",
      edit: "Editar",
      personalInfo: "Información Personal",
      changePassword: "Cambiar Contraseña",
      paymentMethods: "Métodos de Pago",
      savedMethods: "MÉTODOS GUARDADOS",
      addPaymentMethod: "Agregar Método de Pago",
      currency: "Moneda",
      accountActions: "ACCIONES DE LA CUENTA",
      sessionExpired: "Sesión Expirada",
      pleaseLogInAgain: "Por favor, inicie sesión de nuevo.",
      signOutConfirm: "¿Estás seguro de que quieres cerrar sesión?",
      deleteAccountConfirm:
        "¿Estás seguro de que deseas eliminar permanentemente tu cuenta?",
      delete: "Eliminar",
      error: "Error",
      success: "Éxito",
      couldNotDeleteAccount: "No se pudo eliminar la cuenta.",
      user: "Usuario",
      lightMode: "Modo Claro",
      changeMonthlyIncome: "Cambiar Ingreso Mensual",
      monthlyIncomeDesc:
        "Ingresa tu nuevo ingreso mensual para el cálculo del presupuesto.",
      monthlyIncomePlaceholder: "$ 0.00",
      currentName: "Nombre Completo",
      currentMail: "Correo Electrónico",
      currentPassword: "Contraseña Actual",
      currentPasswordPlaceholder: "Ingresa la contraseña actual",
      newPassword: "Nueva Contraseña",
      newPasswordPlaceholder: "Ingresa la nueva contraseña",
      confirmNewPassword: "Confirmar Nueva Contraseña",
      updatePassword: "Actualizar Contraseña",
      save: "Guardar",
      passwordChanged: "Tu contraseña ha sido cambiada con éxito.",
      couldNotUpdateIncome: "Error al actualizar el ingreso mensual.",
      couldNotChangePassword:
        "Error al cambiar la contraseña. Verifica tu contraseña actual.",
      invalidIncome: "Por favor, ingresa un monto de ingreso válido.",
      wageRaise: "Cambio de Ingreso Mensual",
      typoFix: "Corregir Error Tipográfico",

      // Pantalla Resumen
      overview: "RESUMEN",
      hello: "Hola",
      takeCareFinances: "¡Cuida bien de tus finanzas!",
      totalBalance: "SALDO TOTAL",
      investmentPot: "Fondo de Inversión",
      income: "INGRESOS",
      expenses: "GASTOS",
      spendingTrend: "Tendencia de Gastos",
      recent: "Recientes",
      seeAll: "Ver todos",
      deposit: "Depósito",
      cash: "Efectivo",
      card: "Tarjeta",
      noCardsAvailable: "(Sin tarjetas disponibles)",
      cardInUseWarning:
        "Esta tarjeta está asociada a otros gastos. ¿Estás seguro de que deseas eliminarla?",
      deleteCardConfirmation:
        "¿Estás seguro de que deseas eliminar esta tarjeta?",
      deletedCard: "Tarjeta Eliminada",

      // Categorías
      food: "Alimentación",
      housing: "Vivienda",
      transportation: "Transporte",
      entertainment: "Entretenimiento",
      fixed_expenses: "Gastos Fijos",
      healthcare: "Salud",
      clothing: "Ropa",
      pet: "Mascotas",
      travel: "Viajes",
      others: "Otros",

      // Modal de Tarjetas
      registeredCards: "Tarjetas Registradas",
      addCard: "Agregar Tarjeta",
      editCard: "Editar Tarjeta",
      updateCard: "Actualizar Tarjeta",
      noCardsRegistered: "No hay tarjetas registradas",
      close: "Cerrar",
      back: "Atrás",
      newCard: "Nueva Tarjeta",
      cardName: "Nombre de la Tarjeta",
      cardNamePlaceholder: "ej. Santander, BBVA",
      enterCardName: "Por favor, ingresa un nombre para la tarjeta.",
      failedToFetchCards: "Error al obtener las tarjetas registradas.",
      failedToSaveCard: "Error al guardar los detalles de la tarjeta.",
      failedToDeleteCard: "Error al eliminar la tarjeta.",
      cardType: "Tipo de Tarjeta",
      saveCard: "Guardar Tarjeta",
      credit: "Crédito",
      debit: "Débito",
      date: "Fecha",

      // Modal de Ingresos
      addIncomeDeposit: "Agregar Ingreso",
      sourceDescription: "Fuente / Descripción",
      amount: "Monto",
      addToIncome: "Agregar a Ingresos",

      // Modal de Gastos
      addNewExpense: "Agregar Nuevo Gasto",
      description: "Descripción",
      descriptionPlaceholder: "ej. Compras del Supermercado",
      details: "Detalles",
      category: "Categoría",
      paymentMethod: "Método de Pago",
      selectCard: "Seleccionar Tarjeta",
      repeat: "Repetir",
      markAsPaid: "Marcar como Pagado",
      saveExpense: "Guardar Gasto",
      none: "Ninguno",
      weekly: "Semanal",
      monthly: "Mensual",
      yearly: "Anual",

      // Pantalla de Transacciones
      transactions: "Transacciones",
      records: "registros",
      in: "ENTRADA",
      out: "SALIDA",
      net: "NETO",
      all: "Todos",
      allPaymentMethods: "Todos los Métodos de Pago",
      income_transaction: "Depósito",
      expense: "Gasto",
      incomeDeposit: "Depósito de Ingreso",
      noTransactionsRegistered: "No hay transacciones registradas.",

      // Análisis
      analytics: "Visión Analítica",
      spendingInsights: "Análisis de gastos",
      incomeVsExpenses: "Ingresos vs. Gastos",
      last6Months: "Últimos 6 meses",
      paymentMethodBreakdown: "Desglose por Método de Pago",
      cardVsCash: "Tarjeta vs. Efectivo",
      cardExpenses: "Gastos con Tarjeta",
      cashExpenses: "Gastos en Efectivo",
      ofTotal: "del total",
      spendingByCategory: "Gastos por Categoría",
      total: "Total",
      noExpenses: "Sin Gastos",
      netSavingsTrend: "Tendencia de Ahorro Neto",
      netSavings: "AHORRO NETO",
      health: "Salud",
      shopping: "Compras",
      general: "General",
      cardsUsage: "Desglose por Tarjeta",
      savingsAnalysis: "Resumen Financiero y Ahorros",
      incomeVsExpenseDetails: "Análisis de Ingresos vs. Gastos",
      paymentAnalysis: "Análisis Detallado de Métodos de Pago",
      categoryAnalysis: "Análisis del Desglose por Categoría",
      savedSoFar: "Total Ahorrado Hasta Ahora (6 Meses)",
      avgPerMonth: "Ahorro mensual promedio",
      monthlySavingsHistory: "Historial de Ahorro de 6 Meses",
      savingsTip: "Tasa de Ahorro",
      ofIncomeSavedThisMonth: "del ingreso ahorrado este mes.",
      currentMonthNet: "Saldo Neto del Mes Seleccionado",
      comparisonSummary: "Resumen del Período",
      totalIncome: "Ingreso Total (6 Meses)",
      totalExpenses: "Gastos Totales (6 Meses)",
      highestSavingsMonth: "Mes de Mayor Ahorro",
      methodBreakdown: "Resumen de Pagos",
      categoryDistribution: "Análisis por Categoría",
      noCategoryData: "No se encontraron registros de gastos por categoría.",
      creditVsDebit: "Desglose Crédito vs. Débito",
      creditExpenses: "Tarjetas de Crédito",
      debitExpenses: "Tarjetas de Débito",
    },
  },
  ja: {
    translation: {
      // ログイン & 登録
      missingInformation: "入力エラー",
      fillAllFields: "すべての必須項目を入力してください。",
      weakPassword: "パスワードが弱すぎます",
      weakPasswordDesc: "パスワードは8文字以上で入力してください。",
      passwordMismatch: "パスワード不一致",
      passwordsDoNotMatch:
        "パスワードが一致しません。確認してやり直してください。",
      termsRequired: "規約への同意が必要",
      agreeTermsToProceed: "続行するには利用規約に同意してください。",
      createYourAccount: "アカウントを\n作成",
      startTrackingExpenses: "今日から支出管理を始めましょう。",
      fullName: "氏名",
      emailLabel: "メールアドレス",
      monthlyIncomeLabel: "月収（給与 / 収入）",
      passwordLabel: "パスワード",
      confirmPasswordLabel: "パスワード（確認）",
      hide: "非表示",
      show: "表示",
      agreeTo: "",
      termsOfService: "利用規約",
      and: "および",
      privacyPolicy: "プライバシーポリシー",
      createAccount: "アカウント作成",
      alreadyHaveAccount: "すでにアカウントをお持ちですか？ ",
      signIn: "ログイン",
      welcome: "Ledger\n",
      signInToAccount: "個人財務管理・支出トラッキング",
      forgotPassword: "パスワードをお忘れですか？",
      orContinueWith: "または以下で継続",
      dontHaveAccount: "アカウントをお持ちでないですか？ ",
      signUp: "新規登録",
      fullNamePlaceholder: "氏名を入力してください",
      emailPlaceholder: "メールアドレスを入力してください",
      incomePlaceholder: "例: 300000",
      passwordPlaceholder: "8文字以上",
      loginPasswordPlaceholder: "パスワードを入力",
      confirmPasswordPlaceholder: "パスワードを再入力",
      forgotPasswordSubtitle:
        "パスワード再設定コードを受け取るためにメールアドレスを入力してください。",
      resetCodeSent:
        "アカウントが存在する場合、パスワード再設定のリンク/コードが送信されました。",
      genericError: "エラーが発生しました。もう一度お試しください。",
      sendResetCode: "再設定コードを送信",
      ok: "OK",

      // パスワード再設定
      resetPassword: "パスワードの再設定",
      setNewPassword: "新しいパスワードの設定",
      resetPasswordSubtitle:
        "メールに送信された6桁のコードを入力してください。",
      setNewPasswordSubtitle:
        "コードが確認されました。新しいパスワードを入力してください。",
      verificationCode: "認証コード",
      enterVerificationCode: "認証コードを入力してください。",
      enterNewPassword: "新しいパスワードを入力してください。",
      passwordResetSuccess: "パスワードが正常に更新されました！",
      verifyCode: "コードを検証",
      rememberPassword: "パスワードを覚えていますか？",

      // 設定と環境設定
      settings: "設定",
      account: "アカウント",
      preferences: "環境設定",
      language: "言語",
      monthlyBudget: "毎月の予算",
      darkMode: "ダークモード",
      signOut: "サインアウト",
      deleteAccount: "アカウント削除",
      selectLanguage: "言語を選択",
      cancel: "キャンセル",
      edit: "編集",
      personalInfo: "個人情報",
      changePassword: "パスワード変更",
      paymentMethods: "支払い方法",
      savedMethods: "保存された支払い方法",
      addPaymentMethod: "支払い方法を追加",
      currency: "通貨",
      accountActions: "アカウントの操作",
      sessionExpired: "セッション期限切れ",
      pleaseLogInAgain: "再度ログインしてください。",
      signOutConfirm: "本当にサインアウトしますか？",
      deleteAccountConfirm: "本当にアカウントを永久に削除しますか？",
      delete: "削除",
      error: "エラー",
      success: "成功",
      couldNotDeleteAccount: "アカウントを削除できませんでした。",
      user: "ユーザー",
      lightMode: "ライトモード",
      changeMonthlyIncome: "月収の変更",
      monthlyIncomeDesc: "予算計算のための新しい月収を入力してください。",
      monthlyIncomePlaceholder: "￥ 0",
      currentName: "氏名",
      currentMail: "メールアドレス",
      currentPassword: "現在のパスワード",
      currentPasswordPlaceholder: "現在のパスワードを入力",
      newPassword: "新しいパスワード",
      newPasswordPlaceholder: "新しいパスワードを入力",
      confirmNewPassword: "新しいパスワード（確認）",
      updatePassword: "パスワードを更新",
      save: "保存",
      passwordChanged: "パスワードが正常に変更されました。",
      couldNotUpdateIncome: "月収の更新に失敗しました。",
      couldNotChangePassword:
        "パスワードの変更に失敗しました。現在のパスワードを確認してください。",
      invalidIncome: "有効な金額を入力してください。",
      wageRaise: "月収の変更",
      typoFix: "誤字の修正",

      // 概要画面
      overview: "概要",
      hello: "こんにちは",
      takeCareFinances: "資産管理をしっかり行いましょう！",
      totalBalance: "総残高",
      investmentPot: "投資資金",
      income: "収入",
      expenses: "支出",
      spendingTrend: "支出傾向",
      recent: "最近の履歴",
      seeAll: "すべて見る",
      deposit: "入金",
      cash: "現金",
      card: "カード",
      noCardsAvailable: "（利用可能なカードがありません）",
      cardInUseWarning:
        "このカードは他の支出に関連付けられています。本当に削除しますか？",
      deleteCardConfirmation: "本当にこのカードを削除しますか？",
      deletedCard: "削除されたカード",

      // カテゴリ
      food: "食費",
      housing: "住居費",
      transportation: "交通費",
      entertainment: "娯楽費",
      fixed_expenses: "固定費",
      healthcare: "医療・健康",
      clothing: "衣類",
      pet: "ペット",
      travel: "旅行",
      others: "その他",

      // カード管理モーダル
      registeredCards: "登録済みカード",
      addCard: "カードを追加",
      editCard: "カードを編集",
      updateCard: "カード情報を更新",
      noCardsRegistered: "登録済みのカードはありません",
      close: "閉じる",
      back: "戻る",
      newCard: "新しいカード",
      cardName: "カード名",
      cardNamePlaceholder: "例: 楽天カード, 三井住友カード",
      enterCardName: "カード名を入力してください。",
      failedToFetchCards: "登録済みカードの取得に失敗しました。",
      failedToSaveCard: "カード情報の保存に失敗しました。",
      failedToDeleteCard: "カードの削除に失敗しました。",
      cardType: "カードの種類",
      saveCard: "カードを保存",
      credit: "クレジット",
      debit: "デビット",
      date: "日付",

      // 収入モーダル
      addIncomeDeposit: "収入の追加",
      sourceDescription: "収入源 / 説明",
      amount: "金額",
      addToIncome: "収入に追加",

      // 支出モーダル
      addNewExpense: "新しい支出を追加",
      description: "説明",
      descriptionPlaceholder: "例: 食料品の買い出し",
      details: "詳細",
      category: "カテゴリ",
      paymentMethod: "支払い方法",
      selectCard: "カードを選択",
      repeat: "繰り返し",
      markAsPaid: "支払済みとしてマーク",
      saveExpense: "支出を保存",
      none: "なし",
      weekly: "毎週",
      monthly: "毎月",
      yearly: "毎年",

      // 取引履歴画面
      transactions: "取引履歴",
      records: "件の記録",
      in: "収入",
      out: "支出",
      net: "収支",
      all: "すべて",
      allPaymentMethods: "すべての支払い方法",
      income_transaction: "入金",
      expense: "支出",
      incomeDeposit: "入金",
      noTransactionsRegistered: "登録された取引はありません。",

      // 分析
      analytics: "分析視点",
      spendingInsights: "支出インサイト",
      incomeVsExpenses: "収入 vs 支出",
      last6Months: "過去6ヶ月",
      paymentMethodBreakdown: "支払い方法の内訳",
      cardVsCash: "カード vs 現金",
      cardExpenses: "カード支出",
      cashExpenses: "現金支出",
      ofTotal: "の合計",
      spendingByCategory: "カテゴリ別支出",
      total: "合計",
      noExpenses: "支出なし",
      netSavingsTrend: "純貯蓄傾向",
      netSavings: "純貯蓄",
      health: "健康・医療",
      shopping: "買い物",
      general: "全般",
      cardsUsage: "カード別内訳",
      savingsAnalysis: "貯蓄および財務サマリー",
      incomeVsExpenseDetails: "収入 vs 支出のインサイト",
      paymentAnalysis: "支払い方法の詳細分析",
      categoryAnalysis: "カテゴリ別内訳分析",
      savedSoFar: "これまでの総貯蓄額（6ヶ月）",
      avgPerMonth: "月平均貯蓄額",
      monthlySavingsHistory: "6ヶ月間の貯蓄履歴",
      savingsTip: "貯蓄率",
      ofIncomeSavedThisMonth: "（今月の収入に占める貯蓄の割合）",
      currentMonthNet: "選択した月の純収支",
      comparisonSummary: "期間サマリー",
      totalIncome: "総収入（6ヶ月）",
      totalExpenses: "総支出（6ヶ月）",
      highestSavingsMonth: "最高貯蓄月",
      methodBreakdown: "支払い概要",
      categoryDistribution: "カテゴリ分析",
      noCategoryData: "カテゴリ別の支出記録が見つかりません。",
      creditVsDebit: "クレジット vs デビットの内訳",
      creditExpenses: "クレジットカード",
      debitExpenses: "デビットカード",
    },
  },
  ko: {
    translation: {
      // Login & Registration
      missingInformation: "입력 정보 누락",
      fillAllFields: "모든 필수 항목을 입력해 주세요.",
      weakPassword: "취약한 비밀번호",
      weakPasswordDesc: "비밀번호는 최소 8자 이상이어야 합니다.",
      passwordMismatch: "비밀번호 불일치",
      passwordsDoNotMatch:
        "비밀번호가 일치하지 않습니다. 확인 후 다시 시도해 주세요.",
      termsRequired: "약관 동의 필요",
      agreeTermsToProceed: "진행하려면 이용약관에 동의해 주세요.",
      createYourAccount: "계정을\n생성하세요.",
      startTrackingExpenses: "오늘부터 지출 관리를 시작해 보세요.",
      fullName: "이름",
      emailLabel: "이메일",
      monthlyIncomeLabel: "월 소득 (급여 / 수입)",
      passwordLabel: "비밀번호",
      confirmPasswordLabel: "비밀번호 확인",
      hide: "숨기기",
      show: "표시",
      agreeTo: "다음 항목에 동의합니다: ",
      termsOfService: "이용약관",
      and: " 및 ",
      privacyPolicy: "개인정보 처리방침",
      createAccount: "계정 생성",
      alreadyHaveAccount: "이미 계정이 있으신가요? ",
      signIn: "로그인",
      welcome: "Ledger\n",
      signInToAccount: "개인 자산 관리 및 지출 추적",
      forgotPassword: "비밀번호를 잊으셨나요?",
      orContinueWith: "또는 다음으로 계속",
      dontHaveAccount: "계정이 없으신가요? ",
      signUp: "회원가입",
      fullNamePlaceholder: "이름을 입력하세요",
      emailPlaceholder: "이메일 주소를 입력하세요",
      incomePlaceholder: "예: 3000000",
      passwordPlaceholder: "최소 8자 이상",
      loginPasswordPlaceholder: "비밀번호 입력",
      confirmPasswordPlaceholder: "비밀번호 재입력",
      forgotPasswordSubtitle:
        "비밀번호 재설정 코드를 받으려면 이메일 주소를 입력해 주세요.",
      resetCodeSent:
        "계정이 존재하는 경우, 비밀번호 재설정 링크/코드가 전송되었습니다.",
      genericError: "문제가 발생했습니다. 다시 시도해 주세요.",
      sendResetCode: "재설정 코드 전송",
      ok: "확인",

      // 비밀번호 재설정
      resetPassword: "비밀번호 재설정",
      setNewPassword: "새 비밀번호 설정",
      resetPasswordSubtitle: "이메일로 전송된 6자리 코드를 입력해 주세요.",
      setNewPasswordSubtitle:
        "인증되었습니다! 아래에 새 비밀번호를 입력해 주세요.",
      verificationCode: "인증 코드",
      enterVerificationCode: "인증 코드를 입력해 주세요.",
      enterNewPassword: "새 비밀번호를 입력해 주세요.",
      passwordResetSuccess: "비밀번호가 성공적으로 업데이트되었습니다!",
      verifyCode: "코드 인증",
      rememberPassword: "비밀번호가 기억나셨나요?",

      // Settings & Preferences
      settings: "설정",
      account: "계정",
      preferences: "환경설정",
      language: "언어",
      monthlyBudget: "월 예산",
      darkMode: "다크 모드",
      signOut: "로그아웃",
      deleteAccount: "계정 삭제",
      selectLanguage: "언어 선택",
      cancel: "취소",
      edit: "편집",
      personalInfo: "개인정보",
      changePassword: "비밀번호 변경",
      paymentMethods: "결제 수단",
      savedMethods: "저장된 결제 수단",
      addPaymentMethod: "결제 수단 추가",
      currency: "통화",
      accountActions: "계정 작업",
      sessionExpired: "세션 만료",
      pleaseLogInAgain: "다시 로그인해 주세요.",
      signOutConfirm: "정말 로그아웃하시겠습니까?",
      deleteAccountConfirm: "정말 계정을 영구적으로 삭제하시겠습니까?",
      delete: "삭제",
      error: "오류",
      success: "성공",
      couldNotDeleteAccount: "계정을 삭제할 수 없습니다.",
      user: "사용자",
      lightMode: "라이트 모드",
      changeMonthlyIncome: "월 소득 변경",
      monthlyIncomeDesc: "예산 계산을 위한 새로운 월 소득을 입력해 주세요.",
      monthlyIncomePlaceholder: "₩ 0",
      currentName: "이름",
      currentMail: "이메일",
      currentPassword: "현재 비밀번호",
      currentPasswordPlaceholder: "현재 비밀번호 입력",
      newPassword: "새 비밀번호",
      newPasswordPlaceholder: "새 비밀번호 입력",
      confirmNewPassword: "새 비밀번호 확인",
      updatePassword: "비밀번호 업데이트",
      save: "저장",
      passwordChanged: "비밀번호가 성공적으로 변경되었습니다.",
      couldNotUpdateIncome: "월 소득 업데이트에 실패했습니다.",
      couldNotChangePassword:
        "비밀번호 변경에 실패했습니다. 현재 비밀번호를 확인해 주세요.",
      invalidIncome: "올바른 소득 금액을 입력해 주세요.",
      wageRaise: "월 소득 변경",
      typoFix: "오타 수정",

      // Overview Screen
      overview: "개요",
      hello: "안녕하세요",
      takeCareFinances: "자산을 현명하게 관리하세요!",
      totalBalance: "총 잔액",
      investmentPot: "투자금",
      income: "수입",
      expenses: "지출",
      spendingTrend: "지출 추이",
      recent: "최근 내역",
      seeAll: "모두 보기",
      deposit: "입금",
      cash: "현금",
      card: "카드",
      noCardsAvailable: "(사용 가능한 카드 없음)",
      cardInUseWarning:
        "이 카드는 다른 지출 내역과 연결되어 있습니다. 정말 삭제하시겠습니까?",
      deleteCardConfirmation: "정말 이 카드를 삭제하시겠습니까?",
      deletedCard: "삭제된 카드",

      // Categories
      food: "식비",
      housing: "주거비",
      transportation: "교통비",
      entertainment: "문화/유흥",
      fixed_expenses: "고정 지출",
      healthcare: "의료/건강",
      clothing: "의류",
      pet: "반려동물",
      travel: "여행",
      others: "기타",

      // Card Management Modal
      registeredCards: "등록된 카드",
      addCard: "카드 추가",
      editCard: "카드 편집",
      updateCard: "카드 업데이트",
      noCardsRegistered: "등록된 카드가 없습니다",
      close: "닫기",
      back: "뒤로",
      newCard: "새 카드",
      cardName: "카드 이름",
      cardNamePlaceholder: "예: 현대카드, 신한카드",
      enterCardName: "카드 이름을 입력해 주세요.",
      failedToFetchCards: "등록된 카드를 불러오지 못했습니다.",
      failedToSaveCard: "카드 정보를 저장하지 못했습니다.",
      failedToDeleteCard: "카드를 삭제하지 못했습니다.",
      cardType: "카드 종류",
      saveCard: "카드 저장",
      credit: "신용",
      debit: "체크",

      // Income Modal
      addIncomeDeposit: "수입 추가",
      sourceDescription: "출처 / 설명",
      amount: "금액",
      addToIncome: "수입에 추가",

      // Expense Modal
      addNewExpense: "새 지출 추가",
      description: "설명",
      descriptionPlaceholder: "예: 장보기",
      details: "상세 정보",
      category: "카테고리",
      paymentMethod: "결제 수단",
      selectCard: "카드 선택",
      repeat: "반복",
      markAsPaid: "결제 완료 표시",
      saveExpense: "지출 저장",
      none: "없음",
      weekly: "매주",
      monthly: "매월",
      yearly: "매년",
      date: "날짜",

      // Transactions Screen
      transactions: "거래 내역",
      records: "건의 기록",
      in: "수입",
      out: "지출",
      net: "순수입",
      all: "전체",
      allPaymentMethods: "모든 결제 수단",
      income_transaction: "수입",
      expense: "지출",
      incomeDeposit: "수입 입금",
      noTransactionsRegistered: "등록된 거래 내역이 없습니다.",

      // Analytics
      analytics: "분석 시각",
      spendingInsights: "지출 분석",
      incomeVsExpenses: "수입 대 지출",
      last6Months: "최근 6개월",
      paymentMethodBreakdown: "결제 수단별 내역",
      cardVsCash: "카드 대 현금",
      cardExpenses: "카드 지출",
      cashExpenses: "현금 지출",
      ofTotal: "전체의",
      spendingByCategory: "카테고리별 지출",
      total: "합계",
      noExpenses: "지출 없음",
      netSavingsTrend: "순저축 추이",
      netSavings: "순저축",
      health: "건강",
      shopping: "쇼핑",
      general: "일반",
      cardsUsage: "카드별 내역",
      savingsAnalysis: "저축 및 재정 요약",
      incomeVsExpenseDetails: "수입 대 지출 상세 분석",
      paymentAnalysis: "결제 수단 심층 분석",
      categoryAnalysis: "카테고리별 내역 분석",
      savedSoFar: "현재까지 총 저축액 (6개월)",
      avgPerMonth: "월평균 저축액",
      monthlySavingsHistory: "6개월 저축 내역",
      savingsTip: "저축률",
      ofIncomeSavedThisMonth: " (이번 달 수입 대비 저축 비율)",
      currentMonthNet: "선택한 달의 순수입 잔액",
      comparisonSummary: "기간 요약",
      totalIncome: "총 수입 (6개월)",
      totalExpenses: "총 지출 (6개월)",
      highestSavingsMonth: "최고 저축 월",
      methodBreakdown: "결제 개요",
      categoryDistribution: "카테고리 분석",
      noCategoryData: "카테고리별 지출 내역이 없습니다.",
      creditVsDebit: "신용 vs 체크 내역",
      creditExpenses: "신용카드",
      debitExpenses: "체크카드",
    },
  },
  zh: {
    translation: {
      // Login & Registration
      missingInformation: "信息缺失",
      fillAllFields: "请填写所有必填项。",
      weakPassword: "密码强度弱",
      weakPasswordDesc: "密码长度必须至少为 8 个字符。",
      passwordMismatch: "密码不匹配",
      passwordsDoNotMatch: "两次输入的密码不一致，请核对后重试。",
      termsRequired: "需同意条款",
      agreeTermsToProceed: "请同意服务条款以继续。",
      createYourAccount: "创建您的\n账户。",
      startTrackingExpenses: "今天就开始记录您的收支。",
      fullName: "姓名",
      emailLabel: "电子邮箱",
      monthlyIncomeLabel: "月收入（工资 / 薪水）",
      passwordLabel: "密码",
      confirmPasswordLabel: "确认密码",
      hide: "隐藏",
      show: "显示",
      agreeTo: "我同意 ",
      termsOfService: "服务条款",
      and: " 和 ",
      privacyPolicy: "隐私政策",
      createAccount: "创建账户",
      alreadyHaveAccount: "已有账户？ ",
      signIn: "登录",
      welcome: "Ledger\n",
      signInToAccount: "个人财务管理与支出跟踪",
      forgotPassword: "忘记密码？",
      orContinueWith: "或通过以下方式继续",
      dontHaveAccount: "还没有账户？ ",
      signUp: "注册",
      fullNamePlaceholder: "请输入您的姓名",
      emailPlaceholder: "请输入您的电子邮箱地址",
      incomePlaceholder: "例: 3000.00",
      passwordPlaceholder: "至少 8 个字符",
      loginPasswordPlaceholder: "请输入密码",
      confirmPasswordPlaceholder: "再次输入密码",
      forgotPasswordSubtitle: "请输入您的电子邮箱以接收密码重置验证码。",
      resetCodeSent: "如果账户存在，重置密码的链接/验证码已发送。",
      genericError: "出错了，请重试。",
      sendResetCode: "发送重置验证码",
      ok: "确定",

      // 重置密码
      resetPassword: "重置密码",
      setNewPassword: "设置新密码",
      resetPasswordSubtitle: "请输入发送至您邮箱的 6 位数验证码。",
      setNewPasswordSubtitle: "验证码已确认！请在下方输入您的新密码。",
      verificationCode: "验证码",
      enterVerificationCode: "请输入验证码。",
      enterNewPassword: "请输入您的新密码。",
      passwordResetSuccess: "密码更新成功！",
      verifyCode: "验证验证码",
      rememberPassword: "想起密码了？",

      // Settings & Preferences
      settings: "设置",
      account: "账户",
      preferences: "偏好设置",
      language: "语言",
      monthlyBudget: "每月预算",
      darkMode: "深色模式",
      signOut: "退出登录",
      deleteAccount: "注销账户",
      selectLanguage: "选择语言",
      cancel: "取消",
      edit: "编辑",
      personalInfo: "个人信息",
      changePassword: "修改密码",
      paymentMethods: "支付方式",
      savedMethods: "已保存支付方式",
      addPaymentMethod: "添加支付方式",
      currency: "货币",
      accountActions: "账户操作",
      sessionExpired: "会话已过期",
      pleaseLogInAgain: "请重新登录。",
      signOutConfirm: "确定要退出登录吗？",
      deleteAccountConfirm: "确定要永久注销您的账户吗？",
      delete: "注销",
      error: "错误",
      success: "成功",
      couldNotDeleteAccount: "无法注销账户。",
      user: "用户",
      lightMode: "浅色模式",
      changeMonthlyIncome: "修改月收入",
      monthlyIncomeDesc: "请输入新的月收入以用于预算计算。",
      monthlyIncomePlaceholder: "¥ 0.00",
      currentName: "姓名",
      currentMail: "电子邮箱",
      currentPassword: "当前密码",
      currentPasswordPlaceholder: "请输入当前密码",
      newPassword: "新密码",
      newPasswordPlaceholder: "请输入新密码",
      confirmNewPassword: "确认新密码",
      updatePassword: "更新密码",
      save: "保存",
      passwordChanged: "密码已成功修改。",
      couldNotUpdateIncome: "更新月收入失败。",
      couldNotChangePassword: "修改密码失败，请检查当前密码。",
      invalidIncome: "请输入有效的收入金额。",
      wageRaise: "月收入变动",
      typoFix: "错别字修复",

      // Overview Screen
      overview: "概览",
      hello: "你好",
      takeCareFinances: "打理好您的财务！",
      totalBalance: "总资产",
      investmentPot: "投资资金",
      income: "收入",
      expenses: "支出",
      spendingTrend: "支出趋势",
      recent: "最近记录",
      seeAll: "查看全部",
      deposit: "入金",
      cash: "现金",
      card: "银行卡",
      noCardsAvailable: "（无可用银行卡）",
      cardInUseWarning: "该银行卡已关联其他支出记录。确定要删除吗？",
      deleteCardConfirmation: "确定要删除张银行卡吗？",
      deletedCard: "已删除的银行卡",

      // Categories
      food: "餐饮",
      housing: "居住",
      transportation: "交通",
      entertainment: "娱乐",
      fixed_expenses: "固定支出",
      healthcare: "医疗健康",
      clothing: "服饰",
      pet: "宠物",
      travel: "旅游",
      others: "其他",

      // Card Management Modal
      registeredCards: "已绑定银行卡",
      addCard: "添加银行卡",
      editCard: "编辑银行卡",
      updateCard: "更新银行卡",
      noCardsRegistered: "未绑定任何银行卡",
      close: "关闭",
      back: "返回",
      newCard: "新银行卡",
      cardName: "卡片名称",
      cardNamePlaceholder: "例: 招商银行, 微信支付",
      enterCardName: "请输入卡片名称。",
      failedToFetchCards: "获取已绑定银行卡失败。",
      failedToSaveCard: "保存银行卡信息失败。",
      failedToDeleteCard: "删除银行卡失败。",
      cardType: "卡片类型",
      saveCard: "保存卡片",
      credit: "信用卡",
      debit: "借记卡",

      // Income Modal
      addIncomeDeposit: "添加收入",
      sourceDescription: "来源 / 说明",
      amount: "金额",
      addToIncome: "计入收入",

      // Expense Modal
      addNewExpense: "添加新支出",
      description: "说明",
      descriptionPlaceholder: "例: 超市购物",
      details: "详情",
      category: "分类",
      paymentMethod: "支付方式",
      selectCard: "选择银行卡",
      repeat: "重复",
      markAsPaid: "标记为已支付",
      saveExpense: "保存支出",
      none: "无",
      weekly: "每周",
      monthly: "每月",
      yearly: "每年",
      date: "日期",

      // Transactions Screen
      transactions: "交易明细",
      records: "条记录",
      in: "收入",
      out: "支出",
      net: "结余",
      all: "全部",
      allPaymentMethods: "所有支付方式",
      income_transaction: "收入",
      expense: "支出",
      incomeDeposit: "收入存款",
      noTransactionsRegistered: "暂无交易记录。",

      // Analytics
      analytics: "分析视角",
      spendingInsights: "支出洞察",
      incomeVsExpenses: "收入 vs 支出",
      last6Months: "最近 6 个月",
      paymentMethodBreakdown: "支付方式构成",
      cardVsCash: "刷卡 vs 现金",
      cardExpenses: "刷卡支出",
      cashExpenses: "现金支出",
      ofTotal: "占总额",
      spendingByCategory: "分类支出",
      total: "合计",
      noExpenses: "暂无支出",
      netSavingsTrend: "净储蓄趋势",
      netSavings: "净储蓄",
      health: "健康",
      shopping: "购物",
      general: "日常",
      cardsUsage: "银行卡明细",
      savingsAnalysis: "储蓄与财务汇总",
      incomeVsExpenseDetails: "收入 vs 支出洞察",
      paymentAnalysis: "支付方式深度分析",
      categoryAnalysis: "分类明细洞察",
      savedSoFar: "截至目前累计储蓄（6 个月）",
      avgPerMonth: "月均储蓄",
      monthlySavingsHistory: "6 个月储蓄历史",
      savingsTip: "储蓄率",
      ofIncomeSavedThisMonth: "（本月收入储蓄占比）",
      currentMonthNet: "所选月份净结余",
      comparisonSummary: "期间汇总",
      totalIncome: "总收入（6 个月）",
      totalExpenses: "总支出（6 个月）",
      highestSavingsMonth: "最佳储蓄月份",
      methodBreakdown: "支付概览",
      categoryDistribution: "分类分析",
      noCategoryData: "未找到分类支出记录。",
      creditVsDebit: "信用卡 vs 借记卡明细",
      creditExpenses: "信用卡",
      debitExpenses: "借记卡",
    },
  },
  fr: {
    translation: {
      // Login & Registration
      missingInformation: "Informations manquantes",
      fillAllFields: "Veuillez remplir tous les champs obligatoires.",
      weakPassword: "Mot de passe faible",
      weakPasswordDesc: "Le mot de passe doit contenir au moins 8 caractères.",
      passwordMismatch: "Mots de passe non identiques",
      passwordsDoNotMatch:
        "Les mots de passe ne correspondent pas. Veuillez vérifier et réessayer.",
      termsRequired: "Conditions requises",
      agreeTermsToProceed:
        "Veuillez accepter les conditions d'utilisation pour continuer.",
      createYourAccount: "Créez votre\ncompte.",
      startTrackingExpenses: "Commencez à suivre vos dépenses dès aujourd'hui.",
      fullName: "NOM COMPLET",
      emailLabel: "E-MAIL",
      monthlyIncomeLabel: "REVENU MENSUEL (SALAIRE)",
      passwordLabel: "MOT DE PASSE",
      confirmPasswordLabel: "CONFIRMER LE MOT DE PASSE",
      hide: "masquer",
      show: "afficher",
      agreeTo: "J'accepte les ",
      termsOfService: "Conditions d'utilisation",
      and: " et la ",
      privacyPolicy: "Politique de confidentialité",
      createAccount: "Créer un compte",
      alreadyHaveAccount: "Vous avez déjà un compte ? ",
      signIn: "Se connecter",
      welcome: "Ledger\n",
      signInToAccount: "Gestion Financière Personnelle et Suivi des Dépenses",
      forgotPassword: "Mot de passe oublié ?",
      orContinueWith: "ou continuer avec",
      dontHaveAccount: "Vous n'avez pas de compte ? ",
      signUp: "S'inscrire",
      fullNamePlaceholder: "Entrez votre nom complet",
      emailPlaceholder: "Entrez votre adresse e-mail",
      incomePlaceholder: "ex. 3000,00",
      passwordPlaceholder: "Min. 8 caractères",
      loginPasswordPlaceholder: "Entrez votre mot de passe",
      confirmPasswordPlaceholder: "Répétez votre mot de passe",
      forgotPasswordSubtitle:
        "Entrez votre adresse e-mail pour recevoir un code de réinitialisation de mot de passe.",
      resetCodeSent:
        "Si un compte existe, un lien/code de réinitialisation de mot de passe a été envoyé.",
      genericError: "Une erreur s'est produite. Veuillez réessayer.",
      sendResetCode: "Envoyer le code de réinitialisation",
      ok: "OK",

      // Réinitialisation du mot de passe
      resetPassword: "Réinitialiser le mot de passe",
      setNewPassword: "Définir un nouveau mot de passe",
      resetPasswordSubtitle:
        "Entrez le code à 6 chiffres envoyé à votre adresse e-mail.",
      setNewPasswordSubtitle:
        "Code vérifié ! Entrez votre nouveau mot de passe ci-dessous.",
      verificationCode: "CODE DE VÉRIFICATION",
      enterVerificationCode: "Veuillez entrer le code de vérification.",
      enterNewPassword: "Veuillez entrer votre nouveau mot de passe.",
      passwordResetSuccess: "Mot de passe mis à jour avec succès !",
      verifyCode: "Vérifier le code",
      rememberPassword: "Vous vous souvenez de votre mot de passe ?",

      // Settings & Preferences
      settings: "Paramètres",
      account: "COMPTE",
      preferences: "PRÉFÉRENCES",
      language: "Langue",
      monthlyBudget: "Budget mensuel",
      darkMode: "Mode sombre",
      signOut: "Se déconnecter",
      deleteAccount: "Supprimer le compte",
      selectLanguage: "Choisir la langue",
      cancel: "Annuler",
      edit: "Modifier",
      personalInfo: "Informations personnelles",
      changePassword: "Changer le mot de passe",
      paymentMethods: "Moyens de paiement",
      savedMethods: "MÉTHODES ENREGISTRÉES",
      addPaymentMethod: "Ajouter un moyen de paiement",
      currency: "Devise",
      accountActions: "ACTIONS DU COMPTE",
      sessionExpired: "Session expirée",
      pleaseLogInAgain: "Veuillez vous reconnecter.",
      signOutConfirm: "Êtes-vous sûr de vouloir vous déconnecter ?",
      deleteAccountConfirm:
        "Êtes-vous sûr de vouloir supprimer définitivement votre compte ?",
      delete: "Supprimer",
      error: "Erreur",
      success: "Succès",
      couldNotDeleteAccount: "Impossible de supprimer le compte.",
      user: "Utilisateur",
      lightMode: "Mode clair",
      changeMonthlyIncome: "Changer le revenu mensuel",
      monthlyIncomeDesc:
        "Entrez votre nouveau revenu mensuel pour le calcul du budget.",
      monthlyIncomePlaceholder: "0,00 €",
      currentName: "Nom Complet",
      currentMail: "E-mail",
      currentPassword: "Mot de passe actuel",
      currentPasswordPlaceholder: "Entrez le mot de passe actuel",
      newPassword: "Nouveau mot de passe",
      newPasswordPlaceholder: "Entrez le nouveau mot de passe",
      confirmNewPassword: "Confirmer le nouveau mot de passe",
      updatePassword: "Mettre à jour le mot de passe",
      save: "Enregistrer",
      passwordChanged: "Votre mot de passe a été modifié avec succès.",
      couldNotUpdateIncome: "Échec de la mise à jour du revenu mensuel.",
      couldNotChangePassword:
        "Échec de la modification du mot de passe. Veuillez vérifier le mot de passe actuel.",
      invalidIncome: "Veuillez entrer un montant de revenu valide.",
      wageRaise: "Changement de revenu mensuel",
      typoFix: "Correction de faute de frappe",

      // Overview Screen
      overview: "APERÇU",
      hello: "Bonjour",
      takeCareFinances: "Prenez soin de vos finances !",
      totalBalance: "SOLDE TOTAL",
      investmentPot: "Réserve d'investissement",
      income: "REVENUS",
      expenses: "DÉPENSES",
      spendingTrend: "Tendance des dépenses",
      recent: "Récents",
      seeAll: "Voir tout",
      deposit: "Dépôt",
      cash: "Espèces",
      card: "Carte",
      noCardsAvailable: "(Aucune carte disponible)",
      cardInUseWarning:
        "Cette carte est associée à d'autres dépenses. Êtes-vous sûr de vouloir la supprimer ?",
      deleteCardConfirmation:
        "Êtes-vous sûr de vouloir supprimer cette carte ?",
      deletedCard: "Carte supprimée",

      // Categories
      food: "Alimentation",
      housing: "Logement",
      transportation: "Transports",
      entertainment: "Divertissement",
      fixed_expenses: "Charges fixes",
      healthcare: "Santé",
      clothing: "Vêtements",
      pet: "Animaux",
      travel: "Voyages",
      others: "Autres",

      // Card Management Modal
      registeredCards: "Cartes enregistrées",
      addCard: "Ajouter une carte",
      editCard: "Modifier la carte",
      updateCard: "Mettre à jour la carte",
      noCardsRegistered: "Aucune carte enregistrée",
      close: "Fermer",
      back: "Retour",
      newCard: "Nouvelle carte",
      cardName: "Nom de la carte",
      cardNamePlaceholder: "ex. Boursorama, N26",
      enterCardName: "Veuillez entrer un nom de carte.",
      failedToFetchCards: "Échec de la récupération des cartes enregistrées.",
      failedToSaveCard: "Échec de l'enregistrement des détails de la carte.",
      failedToDeleteCard: "Échec de la suppression de la carte.",
      cardType: "Type de carte",
      saveCard: "Enregistrer la carte",
      credit: "Crédit",
      debit: "Débit",

      // Income Modal
      addIncomeDeposit: "Ajouter un revenu",
      sourceDescription: "Source / Description",
      amount: "Montant",
      addToIncome: "Ajouter aux revenus",

      // Expense Modal
      addNewExpense: "Ajouter une nouvelle dépense",
      description: "Description",
      descriptionPlaceholder: "ex. Courses de supermarché",
      details: "Détails",
      category: "Catégorie",
      paymentMethod: "Moyen de paiement",
      selectCard: "Sélectionner une carte",
      repeat: "Répétition",
      markAsPaid: "Marquer comme payé",
      saveExpense: "Enregistrer la dépense",
      none: "Aucune",
      weekly: "Hebdomadaire",
      monthly: "Mensuelle",
      yearly: "Annuelle",
      date: "Date",

      // Transactions Screen
      transactions: "Transactions",
      records: "enregistrements",
      in: "ENTRÉE",
      out: "SORTIE",
      net: "NET",
      all: "Tout",
      allPaymentMethods: "Tous les moyens de paiement",
      income_transaction: "Revenu",
      expense: "Dépense",
      incomeDeposit: "Dépôt de revenu",
      noTransactionsRegistered: "Aucune transaction enregistrée.",

      // Analytics
      analytics: "Vue analytique",
      spendingInsights: "Aperçu des dépenses",
      incomeVsExpenses: "Revenus vs Dépenses",
      last6Months: "6 derniers mois",
      paymentMethodBreakdown: "Répartition par moyen de paiement",
      cardVsCash: "Carte vs Espèces",
      cardExpenses: "Dépenses par carte",
      cashExpenses: "Dépenses en espèces",
      ofTotal: "du total",
      spendingByCategory: "Dépenses par catégorie",
      total: "Total",
      noExpenses: "Aucune dépense",
      netSavingsTrend: "Tendance de l'épargne nette",
      netSavings: "ÉPARGNE NETTE",
      health: "Santé",
      shopping: "Achats",
      general: "Général",
      cardsUsage: "Répartition par carte",
      savingsAnalysis: "Résumé financier et de l'épargne",
      incomeVsExpenseDetails: "Aperçu Revenus vs Dépenses",
      paymentAnalysis: "Analyse approfondie des moyens de paiement",
      categoryAnalysis: "Analyse de la répartition par catégorie",
      savedSoFar: "Total épargné jusqu'à présent (6 mois)",
      avgPerMonth: "Épargne mensuelle moyenne",
      monthlySavingsHistory: "Historique d'épargne sur 6 mois",
      savingsTip: "Taux d'épargne",
      ofIncomeSavedThisMonth: "du revenu épargné ce mois-ci.",
      currentMonthNet: "Solde net du mois sélectionné",
      comparisonSummary: "Résumé de la période",
      totalIncome: "Revenu total (6 mois)",
      totalExpenses: "Dépenses totales (6 mois)",
      highestSavingsMonth: "Meilleur mois d'épargne",
      methodBreakdown: "Aperçu des paiements",
      categoryDistribution: "Analyse par catégorie",
      noCategoryData: "Aucun enregistrement de dépense par catégorie trouvé.",
      creditVsDebit: "Répartition Crédit vs. Débit",
      creditExpenses: "Cartes de crédit",
      debitExpenses: "Cartes de débit",
    },
  },
};
